...
def classificationError(net, dims=8, numExamples=1000):
    ...
    Ts = [oddParity(x) for x in Xs]         # Ground truth

    Ys = [multiFun(x, net)[0] for x in Xs]  # Prediction
...
Xs = filter(lambda X: numOnes(X) not in (0, 1, 2, 6, 7, 8), Xs)
